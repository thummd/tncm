...
def load_model(ckpt_path: str, G: nx.DiGraph,
               latent_dim=64, input_dim=2, seq_length=6, batch_size=500,
               num_flows=5, hidden_dim=128, device="cpu") -> C_VAE:
    prior = {
...
def give_ctf_gt_probs(idx, seed, n_steps, thres_x, thres_y, model, n_samples, device):
    ...
    do = {"0": {idx-1: thres_x}}
    ctf_samp, _ = model.ctf_decoder(do, n_samples, n_steps, seed)
...
def run_eval(args):
    ...
    for s, l1 in enumerate(l1_per_step, start=1):
        q = f"P(Y_(t+1) > {args.thres_y} | do(X_t = {args.thres_x})) @ step={s}"
...
def parse_args():
    p = argparse.ArgumentParser(description="Evaluate Conditional VAE on synthetic DAG counterfactual queries (no training).")
    p.add_argument("--ckpt", required=True, help="Path to checkpoint (.ckpt)")
    p.add_argument("--out", default="results/synth_cvae", help="Output dir for metrics")
    p.add_argument("--relation", default="Experiments/Sythetic_DAG_data/synth_data_back_door_relation.csv")
    p.add_argument("--latent-dim", type=int, default=64)
    p.add_argument("--input-dim", type=int, default=2)
    p.add_argument("--seq-len", type=int, default=6)
    p.add_argument("--batch-size", type=int, default=500)
    p.add_argument("--num-flows", type=int, default=5)
    p.add_argument("--hidden-dim", type=int, default=128)
    p.add_argument("--device", choices=["cpu", "cuda", "auto"], default="cpu")
    p.add_argument("--n-samples", type=int, default=10_000, dest="n_samples")
    p.add_argument("--steps", type=int, default=6)
    p.add_argument("--trials", type=int, default=10)
    p.add_argument("--thres-x", type=float, default=0, dest="thres_x")
    p.add_argument("--thres-y", type=float, default=0, dest="thres_y")
    return p.parse_args()
